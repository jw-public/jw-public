...
describe("AssignmentWeekCopyPaster.copyPasteCalendarWeek()", async function () {
  ...
  it("should be able to copy one assignment", async function () {
    ...
    const copyActionsCollection = new LocalCollection<AssignmentCopyActionDAO>(
      "copyActionsCollection",
    );
    const toBeTested = new AssignmentWeekCopyPaster(
      fixtureCollection,
      copyActionsCollection,
      new JsnLogFactory(),
    );
    ...
    chai.assert.strictEqual(
      ...
      new Date("2023-01-09T12:00:00.000+01:00").toISOString(),
    );
    chai.assert.strictEqual(
      result.end.toISOString(),
      new Date("2023-01-09T14:00:00.000+01:00").toISOString(),
    );
  ...
  it("should be able to copy one assignment", async function () {
    ...
    // Act
    await toBeTested.copyPasteCalendarWeekInGroup({
      groupId: original.group,
      from: {
        calendarWeek: 23,
        year: 2022,
