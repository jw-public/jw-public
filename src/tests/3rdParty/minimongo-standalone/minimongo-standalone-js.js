...
_.extend(Meteor, {
  ...
  _delete: function (obj) {
    ...
    for (var i = stack.length - 1; i >= 0; i--) {
      ...
      if (leaf) leaf = false;
      else for (var other in stack[i][key]) return;
...
else Random = new RandomGenerator([new Date(), height, width, agent, Math.random()]);
...
(function () {
  ...
  gju.drawCircle = function (radiusInMeters, centerPoint, steps) {
    var center = [centerPoint.coordinates[1], centerPoint.coordinates[0]],
      dist = radiusInMeters / 1e3 / 6371, // convert meters to radiant
      radCenter = [gju.numberToRadius(center[0]), gju.numberToRadius(center[1])],
      steps = steps || 15, // 15 sided circle
...
_.extend(LocalCollection.Cursor.prototype, {
  ...
  observeChanges: function (options) {
    ...
    if (!options._allow_unordered && !ordered && (self.skip || self.limit))
      throw new Error(
        "must use ordered observe (ie, 'addedBefore' instead of 'added') with skip or limit",
      );
...
LocalCollection.Cursor.prototype._getRawObjects = function (options) {
  ...
  if (self._selectorId !== undefined) {
    ...
    if (selectedDoc) {
      ...
      else results.set(self._selectorId, selectedDoc);
    }
    return results;
  }
...
LocalCollection.prototype.insert = function (doc, callback) {
  ...
  for (var qid in self.queries) {
    ...
    if (matchResult.result) {
      ...
      else LocalCollection._insertInResults(query, doc);
    }
  }
  ...
  $near: function (operand, valueSelector, matcher, isRoot) {
    ...
    return function (branchedValues) {
      ...
      _.each(branchedValues, function (branch) {
        ...
        if (!branch.arrayIndices) delete result.arrayIndices;
        else result.arrayIndices = branch.arrayIndices;
      });
      return result;
    };
  },
  ...
  $in: {
    compileElementSelector: function (operand) {
      ...
      _.each(operand, function (option) {
        if (option instanceof RegExp) elementMatchers.push(regexpElementMatcher(option));
        else if (isOperatorObject(option)) throw Error("cannot nest $ under $in");
        else elementMatchers.push(equalityElementMatcher(option));
      });
      return function (value) {
  ...
  _generateKeysFromDoc: function (doc, cb) {
    ...
    _.each(self._sortSpecParts, function (spec, whichField) {
      ...
      if (knownPaths) {
        ...
        // non-array field.
        if (
          !_.has(valuesByIndexAndPath[whichField], "") &&
...
LocalCollection._compileProjection = function (fields) {
  ...
  var transform = function (doc, ruleTree) {
    ...
    _.each(ruleTree, function (rule, key) {
      ...
      } else if (details.including) res[key] = EJSON.clone(doc[key]);
      else delete res[key];
    });
    return res;
  };
...
var pathsToTree = function (paths, newLeafFn, conflictFn, tree) {
  ...
  _.each(paths, function (keyPath) {
    ...
    var success = _.all(pathArr.slice(0, -1), function (key, idx) {
      if (!_.has(treePos, key)) treePos[key] = {};
    ...
    if (success) {
      ...
      if (!_.has(treePos, lastKey)) treePos[lastKey] = newLeafFn(keyPath);
      else treePos[lastKey] = conflictFn(treePos[lastKey], keyPath, keyPath);
    }
  });
  return tree;
};
...
LocalCollection._modify = function (doc, mod, options) {
  ...
  if (!isModifier) {
    ...
    for (var k in mod) {
      if (/\./.test(k))
        throw MinimongoError("When replacing document, field name may not contain '.'");
    }
    newDoc = mod;
  } else {
...
var MODIFIERS = {
  ...
  $pushAll: function (target, field, arg) {
    ...
    var x = target[field];
    if (x === undefined) target[field] = arg;
  ...
  $addToSet: function (target, field, arg) {
    ...
    var x = target[field];
    if (x === undefined) target[field] = values;
    else if (!(x instanceof Array))
  ...
  $pop: function (target, field, arg) {
    ...
    else if (!(x instanceof Array)) throw MinimongoError("Cannot apply $pop modifier to non-array");
    else {
      if (typeof arg === "number" && arg < 0) x.splice(0, 1);
      else x.pop();
    }
  },
...
LocalCollection._diffQueryChanges = function (ordered, oldResults, newResults, observer, options) {
  if (ordered) LocalCollection._diffQueryOrderedChanges(oldResults, newResults, observer, options);
  else LocalCollection._diffQueryUnorderedChanges(oldResults, newResults, observer, options);
};
...
LocalCollection._diffObjects = function (left, right, callbacks) {
  _.each(left, function (leftValue, key) {
    if (_.has(right, key)) callbacks.both && callbacks.both(key, leftValue, right[key]);
    else callbacks.leftOnly && callbacks.leftOnly(key, leftValue);
  });
    ...
    observeChangesCallbacks = {
      ...
      movedBefore: function (id, before) {
        ...
        observeCallbacks.movedTo(
          transform(EJSON.clone(self.docs.get(id))),
          from,
          to,
          before || null,
        );
...
LocalCollection._selectorIsIdPerhapsAsObject = function (selector) {
  return (
    ...
    (selector &&
      ...
      selector._id &&
      LocalCollection._selectorIsId(selector._id) &&
      _.size(selector) === 1)
  );
};
