...
import {
  BlueprintMaterializerOptions,
  IBlueprintMaterializer,
} from "./../interfaces/IBlueprintMaterializer";
...
export class BlueprintMaterializer implements IBlueprintMaterializer {
  ...
  private processSingleAssignment(
    element: BlueprintAssignmentDAO,
    options: BlueprintMaterializerOptions,
    group: string,
  ) {
    let startDate = moment(options.weekOfGivenDate)
      .isoWeekday(element.isoWeekday)
