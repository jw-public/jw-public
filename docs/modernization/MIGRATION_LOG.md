...
- ✅ **DI rewrite (ADR 0005)**: InversifyJS 2.0.1 (+ inversify-inject-decorators, reflect-metadata) REMOVED — replaced by a hand-written composition root (`server/services.ts`, `buildServices(collections, emailSender, overrides)`). All 19 classes are decorator-free; tsconfig dropped experimentalDecorators/emitDecoratorMetadata; tests build the same graph against LocalCollections with an overrides object instead of kernel rebinding (old Type symbols live on as test identifiers via `tests/common/serviceSymbols.ts`). Unit tests 101→99 (the two deleted inversify-playground specs tested the framework, not the app).
- ✅ **`typescript` Meteor package ACTIVE** (unblocked by the DI rewrite): Meteor compiles .ts/.tsx directly — the tsc-emit pipeline and its `npm run compile`-after-every-edit trap are GONE (150 emitted .js deleted; tsconfig is now `noEmit` + `esModuleInterop`, `npm run compile` = pure typecheck). Unit tests run on the TS sources via mocha + tsx. Side cleanup: dead hand-written typings for long-removed packages deleted (moment/moment-timezone/remove-markdown etc. now use package-provided types).
- ⛔→ (historisch) **`typescript` Meteor package was BLOCKED**: InversifyJS uses constructor **parameter decorators** (`@inject(...)`) — a TypeScript-only feature Babel cannot compile, and Meteor's typescript package is Babel-based. Prerequisite: migrate DI away from parameter decorators (e.g. property injection or factory bindings). Until then the tsc-emit pipeline stays (`npm run compile` after every TS edit!).
- ✅ **bootbox → React modals + jQuery COMPLETELY REMOVED (branch modals)**: own Promise-based dialog module (`client/react/components/dialogs.tsx`: confirmDialog/promptDialog/contentDialog, BS5 markup, stacking via z-index counter) replaces all 7 bootbox call sites incl. the AssignmentManager shell. With bootbox gone nothing needed jQuery anymore: atmosphere `jquery` package, npm jquery + @types removed; main.ts resize → plain DOM; SmallProgressbar tooltip → window.bootstrap.Tooltip; ResponsiveHelper (probed BS3 hidden-* classes — silently broken since the BS5 sweep!) → matchMedia; dead AssignmentForm.ts (jQuery datetimepicker era, zero importers) deleted. Specs: .bootbox→.app-modal selectors.
- Tier 3 REMAINING (deliberately separate steps): moment→dayjs, SimpleSchema→zod, React 19, inversify 8, chai/mocha/sinon majors, marked 18, jquery 4
